# This migration comes from compass_ae_business_suite (originally 20150112195601)
# This migration comes from compass_ae_business_suite (originally 20150112195601)
class AddPositionFulfillmentBusinessModule

  def self.up

    # create the module
    new_module = BusinessModule.create(
        description: 'Position Fulfillment',
        internal_identifier: 'position_fulfillment',
        data_manager_name: 'PositionFulfillment',
        root_model: 'PositionFulfillment',
        can_create: false,
        can_associate: true,
        is_template: true
    )
    ...
    organizer_view = OrganizerView.new(
        description: 'Position Fulfillment Organizer Template',
        title: 'Position Fulfillment Organizer Template',
    ...
    web_view = WebView.new(
        description: 'Position fulfillment Web View Template',
        title: 'Position fulfillment Web View Template',
        can_edit_detail_view: true,
        can_edit_list_view: true
    )
    ...
    %w{description position_type from_date thru_date}.each do |field_iid|
  ...
  def self.down
    BusinessModule.iid('position_fulfillment').destroy
  end
end
