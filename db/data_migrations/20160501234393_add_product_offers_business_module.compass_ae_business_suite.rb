# This migration comes from compass_ae_business_suite (originally 20141253155027)
class AddProductOffersBusinessModule
  ...
  def self.up
    ...
    new_module = BusinessModule.create(
        ...
        internal_identifier: 'product_offers',
        data_manager_name: 'ProductOffers',
        root_model: 'SimpleProductOffer',
        can_create: true,
        can_associate: true,
        is_template: true
    )
    ...
    organizer_view = OrganizerView.new(
        description: 'ProductOffers Organizer Template',
        title: 'ProductOffers Organizer Template',
    ...
    web_view = WebView.new(
        description: 'ProductOffers Web View Template',
        title: 'ProductOffers Web View Template',
        can_edit_detail_view: true,
        can_edit_list_view: true
    )
    ...
    [
        ...
        ['Product Type', 'product_type_field', 'product_type'],
        ['Price', 'price', 'price'],
        ['From Date', 'valid_from', 'date'],
        ['To Date', 'valid_to', 'date']
    ].each do |args|
      label = args[0]
      field_name = args[1]
      field_type = args[2]
    ...
    %w{description product_type_field price valid_from valid_to}.each do |field_iid|
  ...
  def self.down
    BusinessModule.where(internal_identifier: 'product_offers').first.destroy
  end
end
