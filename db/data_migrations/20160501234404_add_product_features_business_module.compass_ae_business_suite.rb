# This migration comes from compass_ae_business_suite (originally 20150126222907)
class AddProductFeaturesBusinessModule
  ...
  def self.up
    ...
    new_module = BusinessModule.create(
      ...
      internal_identifier: 'product_features',
      data_manager_name: 'ProductFeatures',
      root_model: 'ProductFeature',
      can_create: true,
      can_associate: true,
      is_template: true
    )
    ...
    organizer_view = OrganizerView.new(
      description: 'ProductFeatures Organizer Template',
      title: 'Product Features Organizer Template',
    ...
    web_view = WebView.new(
      description: 'ProductFeatures Web View Template',
      title: 'Product Features Web View Template',
      can_edit_detail_view: true,
      can_edit_list_view: true
    )
    ...
    # add fields
    [
      ['Product Feature Type', 'product_feature_type', 'product_feature_type', []],
      ['Product Feature Value', 'product_feature_value', 'product_feature_value', []],
    ].each do |args|
      label = args[0]
      field_name = args[1]
      field_type = args[2]
    ...
    %w{product_feature_type product_feature_value}.each do |field_iid|
    ...
    unless ProductFeatureInteractionType.iid('invalid')
      p = ProductFeatureInteractionType.new
      p.internal_identifier = 'invalid'
